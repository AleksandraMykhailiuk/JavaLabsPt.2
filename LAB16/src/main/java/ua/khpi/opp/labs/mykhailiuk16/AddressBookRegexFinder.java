...
import ua.khpi.opp.labs.mykhailiuk16.constants.FindingRegex;
...
public class AddressBookRegexFinder {
    private static final String[] kyivstarNumberStartings = {"39", "67", "98"};
        ...
        addressBook.forEach(
                (element) -> {
                    if(Pattern.compile(FindingRegex.STREET_KHARKIV.getValue())
                            .matcher(element.getAddress().toAddressLine()).matches()) { //todo change validation of address
